...
# Homogenous
parameters_grid = [{season: {'beta': limits(np.random.normal(0.00000068, 0.00000068/3.5)),
                    'delta': 1,
                    'phi': (-1*np.random.uniform(20, 45)/52)*2*np.pi,
                    'epsilon': 1}}
                   for i in range(500)]
...
# Save current grid
with open(f'../../data/influenza_model/results/homo_grid_search_2016_2_new.pickle', 'wb') as pickle_out:
...
if __name__ == '__main__':
    ...
    # Saving the results
    with open(f'../../data/influenza_model/results/homo_grid_search_2016_2_new_res.pickle', 'wb') as pickle_out:

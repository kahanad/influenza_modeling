...
parameters_grid = [{2011: {'beta': limits(np.random.normal(0.0017, 0.0017/10)),
                    ...
                    2012: {'beta': limits(np.random.normal(0.00145, 0.00145/10)),
                             'delta': 1,
                             'phi': (-1*np.random.uniform(28, 35)/52)*2*np.pi,
                             'epsilon': 1},
...
if __name__ == '__main__':
    ...
    # Saving the results
    with open(f'../../data/coupled_model/grid_search_homo/grid_search_homo_{season}_m{m}_k{k}_results.pickle', 'wb') as pickle_out:
